...
const ConditionRule: FC<{ type: ConditionType }> = ({ type }) => {
  ...
  const label =
    type === 'linking' ? <Trans>Smarty Tag</Trans> : <Trans>Field</Trans>;
    ...
    <>
      ...
      {Boolean(values.display && values.display !== 'ALWAYS') && (
        <>
          ...
          <Button color="primary" variant="text" onClick={handleAddMore}>
            add more
          </Button>
        </>
      )}
...
const ConditionSettings: FC<Props> = (props) => {
  ...
  const settings: DndItemSetting[] =
    conditionSettings[editorContext.template.id];
  const values =
