...
type Props = RenderProps & React.HTMLAttributes<HTMLDivElement>;
...
const DndItemPreview: React.FC<Props> = React.forwardRef<HTMLDivElement, Props>(
  (
    ...
    const handleSettingsClick = (ev: React.MouseEvent<any>) => {
      handleClick(ev);
      PubSub.publish('component/click', {
        type: 'container',
        data: '__container',
      });
    };
    ...
    const handleDelete = (ev: React.MouseEvent<HTMLButtonElement>) => {
      ev.stopPropagation();
      removeItem({...renderProps, children}, item?.id);
    };
    const isActive = active === item?.id;
    const stateItem = item && state.entities[item.id];
    const parentItem =
      stateItem && (itemsMap[stateItem.parent.id] as DndBlockItem);

    React.useEffect(() => {
      if (item) {
        const subId = PubSub.subscribe(item.id, (data: { data: { item: { id: string; }; }; }) => {
