...
const FormRenderer: FC<FormRendererProps> = ({
  ...
  renderProps.state.items?.map((item) => {
    ...
    const itemValidation = (
      renderProps.itemsMap[stateItem.parent.id] as DndComponentItem
    )?.validationSchema?.(updatedRenderProps);
    if (itemValidation && stateItem.name)
  ...
  const handleSubmit = (value: any, formikHelper: FormikHelpers<any>) =>
    onSubmit?.(value);
