...
import { useDeepCompare } from '@saastack/rehooks';
...
const useStyles = makeStyles(
  ({ palette: { background, divider }, spacing }: Theme) => ({
    root: {
      width: '100%',
      height: '100%',
      overflow: 'hidden',
    ...
    menu: {
      ...
      top: 0,
      bottom: 0,
      borderRight: `1px solid ${alpha(divider, 0.08)}`,
      zIndex: 1,
      backgroundColor: background.paper,
      overflow: 'auto',
    },
    preview: {
      ...
      flex: 1,
      width: `calc(100% - ${spacing(65)})`,
      zIndex: 0,
    },
    preferences: {
      flex: `0 0 ${spacing(40)}`,
      width: spacing(40),
      borderLeft: `1px solid ${alpha(divider, 0.08)}`,
      zIndex: 1,
      backgroundColor: background.paper,
    },
  })
...
const DndEditor: FC<DndEditorProps> = ({
  value,
  onChange,
  items = [],
  ...
  const fonts = useDeepCompare(
    fontFamily.map((family) => [
      family.id,
      ...fontWeights.map((i) => i.id.toString()),
    ])
  );
  useEffect(() => {
    const fontsWithSizes = fonts.map((fontArray: string | any[]) => {
  ...
  const [state, setState] = useState<DndState>(createDndState(value, template));
  const itemsMap = {
  ...
  return (
    <DndEditorProvider {...editorContextProps}>{children}</DndEditorProvider>
  );
};
