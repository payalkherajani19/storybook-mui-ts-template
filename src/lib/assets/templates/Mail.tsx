...
const Background: FC<Props> = ({ renderProps, children }) => {
  ...
  return (
    <div style={stateItem?.values?.outer?.style} className={classes.background}>
      <div style={stateItem?.values?.inner?.style} className={classes.email}>
        <span
          className={clsx(classes.backgroundTag, 'background-tag', classes.tag)}
        >
...
export default {
  ...
  render: (renderProps, children) => (
    <Background renderProps={renderProps} children={children} />
  ),
