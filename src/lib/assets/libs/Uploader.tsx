...
> = ({ value, onChange, name, error, ...props }) => {
  const fileRef = useRef<globalThis.File>();
  const [progress, setProgress] = useState(false);
  const [, forceUpdate] = useState({});
  const errorRef = useRef('');
  const [visible, setVisible] = useState(false);
  const classes = useStyles();
  const handleChangeEvent = (ev: ChangeEvent<HTMLInputElement>) => {
  ...
  const ext = fileRef.current
    ? last(fileRef.current.name.split('.')) ?? ''
    : '';
      ...
      {fileRef.current ? (
        ...
        <>
          ...
          <div className={classes.placeholder}>
            ...
            <Typography variant="caption">
              <Trans>
                Drag and drop or click to upload, file size should be less than
                3MB
              </Trans>
            </Typography>
          </div>
