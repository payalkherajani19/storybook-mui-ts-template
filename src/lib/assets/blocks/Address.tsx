...
export default {
      ...
      <Grid container alignItems="stretch" spacing={2}>
        ...
        <Grid item xs={6}>
          {TextInput.render(
            renderProps,
            'state',
            getFormikKey(renderProps, 'state')
          )}
        </Grid>
        <Grid item xs={6}>
          {TextInput.render(
            renderProps,
            'postal',
            getFormikKey(renderProps, 'postal')
          )}
        </Grid>
        <Grid item xs={6}>
          {Country.render(
            renderProps,
            'country',
            getFormikKey(renderProps, 'country')
          )}
        </Grid>
      </Grid>
    );
  ...
  validationSchema: (renderProps: any) => {
    const schema: any = {};
    schema[getFormikKey(renderProps, 'address-line-1')] =
      TextInput.validationSchema?.(renderProps, 'address-line-1');
    schema[getFormikKey(renderProps, 'address-line-2')] =
      TextInput.validationSchema?.(renderProps, 'address-line-2');
