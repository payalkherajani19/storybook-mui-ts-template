...
export default {
  ...
  render: (renderProps) =>
    FileUpload.render(
      renderProps,
      'file-upload',
      getFormikKey(renderProps, 'file-upload')
    ),
  validationSchema: (renderProps: any) => {
    const schema: any = {};
    schema[getFormikKey(renderProps, 'file-upload')] =
      FileUpload.validationSchema?.(renderProps, 'file-upload');
    return schema;
  },
  export: () => '',
  image:
  ...
  initialValues: {
    'file-upload': FileUpload.initialValues,
  },
  settings: [
    {
      id: 'file-upload',
