  ...
  return (
    <div className={classes.label}>
      ...
      <div className="subtitle">
        <Trans>
          {' '}
          Control the background, alignment and padding of the selected block{' '}
        </Trans>
      </div>
    </div>
  );
...
const defaultContainer: DndContainerItem = {
  render: (renderProps: RenderProps, children?: ReactNode) => {
    if (!renderProps.item) {
      return null;
    }
    const state =
      renderProps.state.entities[renderProps.item.id]?.values?.__container;
