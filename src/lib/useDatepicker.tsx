...
import { StaticDatePickerProps as MuiDatePickerProps } from '@mui/lab'
import { StaticDateRangePickerProps as MuiDateRangePickerProps } from '@mui/lab'
import { PickersDay as Day } from '@mui/x-date-pickers/PickersDay'
import moment, { Moment } from 'moment-timezone'
import { StaticDatePicker, StaticDateRangePicker } from '@mui/x-date-pickers-pro'
...
import { DateRangePickerDayProps as DateRangeDayProps } from '@mui/x-date-pickers-pro/DateRangePickerDay'
...
const useDatepicker = (
    ...
    const startDate = useMemo(() => {
        ...
        if (clonedProps.minDate && !selectedDates.length) {
            const minMomentDate = utils.startOfDay(moment.tz(clonedProps.minDate, timezone))
        ...
        if (clonedProps.maxDate && !selectedDates.length) {
            const maxMomentDate = utils.startOfDay(moment.tz(clonedProps.maxDate, timezone))
    ...
    const formattedDates = useMemo(() => selectedDates.map((date) => date.format(dateFormat)), [
        selectedDates,
        timezone
    ])
            ...
            switch (mode) {
                case 'multiple':
                    ...
                    value = xor(
                        ...
                        date ? [utils.startOfDay(moment.tz(date, timezone)).toISOString()] : []
                    )
