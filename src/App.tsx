...
const customItems: DndItem[] = [
  ...
  },
  {
    ...
    export: (renderProps: RenderProps) => {
      ...
      const stateImage =
        renderProps.state.entities[renderProps.item.id].values['image-1'];
      const stateText =
        renderProps.state.entities[renderProps.item.id].values['text-1'];
      return `
            {% if business.logo == "" %}
            ...
            {% endif %}
            {% if business.logo != "" %}
            <img src="{{business.logo}}"  style="${styleToCss(
              stateImage.style
            )}" alt="">
            {% endif %}
        `;
    ...
    render: (renderProps) => {
      return (
        <>
          ...
          <p style={{ textAlign: 'center' }}>
            {Button.render(renderProps, 'button-2')}
          </p>
...
function App() {
  ...
  const theme = useMemo(
    () => createTheme({ typography: { fontFamily: '"Poppins", sans-serif' } }),
    []
  );
